...
export async function userUpdateController(req: any, res: Response) {
  const { role, email, address, name, surname, birthdate } = req.body.user
  ...
  try {
    ...
    if(birthdate) await db.update(usersTable).set({ birthdate: birthdate }).where(eq(usersTable.email, email))
